...
<head>
    <script>
		...
		$(document).on('change', '#gruproduto', function(){  
		let grupo = $('#gruproduto option:selected').val();
        let p     = '<?=$param;?>';
		let pr    = '<?=$modulo;?>'
		$att('#tabbela0').html('<center><br><br><img src="../imgs/ajax3.gif" alt="consultando"></center>')
			 .load('forms/produtos/frmSelect.php',{ param:p, modulo:pr, codgru:grupo });
        });
    </script>
</head>   
<body onLoad="if(ieBlink){setInterval('doBlink()',450)}" scrollbars="yes">
        ...
        <div class="row modal-header" style="background: #d9edf7; border-radius: 10px 10px 0px 0px; max-height:50px;">
			...
			<h3 class="modal-title" style="font-weight:bold; font-size:20pt; color:#4F94CD">Produtos</h3> 
			...
			?>
				...
				<select class="btn dropdown-toggle btn-default bootstrap-select" name="gruproduto" id="gruproduto" onchange="">
					...
					<table id="tabbela1" class="table-striped table-bordered table-hover table">
						...
						//----------- Conexao a Base de Dados Produtos-----------------------------------------------------------------		
							...
							$cons_produto = $conn->prepare("SELECT 	p.codpro, p.codgru, p.codmar, p.codigo, p.codori, 
																	...
																	g.nome, g.tipo
															FROM cada_produtos AS p 
															INNER JOIN cada_grupos AS g ON g.codgru = p.codgru 
														   ");
	...
	//-------------------------------------------------------------------------------------------------------------------------------------																	
						?>
							<tr>
								...
								<td class='ate900'><center><img class='detalhe' src='<?=$caminho.$prod_imagem;?>'></center></td>
							</tr>
						<?php
							}
						}
